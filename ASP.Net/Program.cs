...
app.UseStaticFiles(); // добавляем в midleware конвеер еще один стандартный компонент (метод расширения -обертка в котором компонент) - обрабатывает запросы к файлам в папке wwwroot
app.Run(async (context) =>
{
...
//    // WriteAsync(): отправляет некоторое содержимое клиенту (http serverу kestrel).
